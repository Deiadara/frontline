/**
 * @frontline/shared: the single source of truth for the Frontline domain model.
 * Every type is co-located with its Zod schema (`type X = z.infer<typeof XSchema>`).
 */
export * from './primitives.js';
export * from './time/zone.js';
// The speed model itself, not only the caps: `roadMinutes` is the one clock every road in the
// game is measured with and the client draws quotes off it. The modules that spend each cap still
// re-export their own, so nothing that already imported one had to move.
export * from './time/speed.js';
export * from './time/cancel.js';
export * from './attributes.js';
export * from './roles.js';
export * from './figures.js';
export * from './resources.js';
export * from './allegiance.js';
export * from './feats/index.js';
export * from './api.feats.js';
export * from './economy/infamy.js';
export * from './economy/notoriety.js';
export * from './economy/payroll.js';
export * from './economy/renown.js';
export * from './economy/state.js';
export * from './progression/index.js';
export * from './delegation/index.js';
export * from './bar/index.js';
export * from './research/index.js';
export * from './modification-rarity.js';
export * from './building/index.js';
export * from './overseer.js';
export * from './commander.js';
export * from './base.js';
export * from './city/index.js';
export * from './crew/index.js';
export * from './factions/index.js';
export * from './live/index.js';
export * from './scouting/index.js';
export * from './spying/index.js';
export * from './moves/index.js';
export * from './tutorial/index.js';
export * from './automations/index.js';
export * from './social/index.js';
export * from './items/index.js';
export * from './blueprints/index.js';
export * from './market/index.js';
export * from './units/index.js';
export * from './raid.js';
export * from './sleepers.js';
export * from './missions.js';
export * from './missions.areas.js';
export * from './missions.ramp.js';
export * from './missions.leading.js';
export * from './missions.offer.js';
export * from './user.js';
export * from './art/prompts.js';
export * from './art/manifest.js';
export * from './art/hero.js';
export * from './art/backdrop.js';
export * from './battle/types.js';
export * from './battle/attrition.js';
export * from './battle/battlefield.js';
export * from './battle/effects.js';
export * from './battle/engine.js';
export * from './battle/forecast.js';
export * from './battle/luck.js';
export * from './battle/matchup.js';
export * from './battle/morale.js';
export * from './battle/report.js';
export * from './battle/rout.js';
export * from './battle/skirmish.js';
export * from './battle/testing.js';
export * from './rng.js';
export * from './battle/schedule.js';
export * from './battle/scheduled.js';
export * from './battle/perimeter.js';
export * from './battle/traps.js';
export * from './battle/analysis.js';
export * from './battle/officer.js';
export * from './battle/boosts.js';
export * from './battle/movement.js';
export * from './api.js';
export * from './api.district.js';
export * from './api.battle.js';
export * from './api.garage.js';
export * from './api.social.js';
export * from './api.profile.js';
export * from './api.accounts.js';
export * from './mvp.js';
