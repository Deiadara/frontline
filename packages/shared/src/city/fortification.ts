 ...
 * Digging in (GDD §A4).
 *
 * Five levels on any place you hold, each costing more materials and more time than the last. What
 * differs between one place and another is not the price — it is what the work is *worth*, and
 ...
 * is marginal. A place that is both hard to take *and* rewards fortifying would be strictly best,
...
/** Level 5 costs ~8.4x level 1 — a real commitment on one place rather than a rounding error. */
...
/**
 * What raising a place **to** `level` costs.
 *
...
/** How long raising a place to `level` takes, in seconds. */
export function fortifySeconds(level: number): number {
  return Math.round(FORTIFY_BASE_SECONDS * FORTIFY_TIME_GROWTH ** (level - 1));
}

/** The level a further order would produce, or `null` when the place is already dug in fully. */
...
/** Everything a fortification row needs to render, for one place at one level. */
export interface FortifyQuote {
  level: number;
  cost: PartialResources;
  seconds: number;
  /** The defence this level takes the place to, in total — not what the level alone adds. */
  bonusPercent: number;
}

export function quoteFortify(place: Place, current: number): FortifyQuote | null {
  ...
  return {
    ...
    bonusPercent: fortifyBonusPercent(place.fortifyDifficulty, level),
  };
}

/**
 * How much harder this place is to take than the bare ground, as a multiplier.
 *
 * Reported rather than folded into `baseDefense` so a place card can say what the *digging* is
 * doing separately from what the ground was worth to begin with.
 */
export function fortifiedDefense(place: Place, level: number): number {
  const ground = PLACE_KIND_CATALOG[place.kind].baseDefense;
  return (
    Math.round(ground * (1 + fortifyBonusPercent(place.fortifyDifficulty, level) / 100) * 10) / 10
  );
}
