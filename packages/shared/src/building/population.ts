 ...
 *
 * `populationCapacity` is the Quarters and the Cistern; `effects.populationBonus` is what the map
 * adds, which is `POPULATION_PER_LOCATION` for every location held plus whatever the handful of
 * locations that house people explicitly give on top.
 */
