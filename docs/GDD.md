...
Source of truth for the feature set the maintainer specified in **MOU-159**. Every requirement the maintainer wrote
...
- **[SPEC]**: written by the maintainer. Do not reinterpret; if it looks wrong, ask.
- **[PROVISIONAL]**: a decision taken by an agent to unblock implementation. The maintainer may override
  at any time; overriding one costs a small refactor, not a redesign.
- **[TODO-LATER]**: explicitly deferred by the maintainer ("we'll work on that later" / "I'll add the new
...
- **A2 [SPEC]** Overall aesthetic: **less futuristic, more dystopic**. Reference is **Zaun / the
...
- **A3 [SPEC]** In NPC battles the main enemy is **the Government**, which is a tyranny. NPC content,
  faction naming, mission fiction and enemy composition all hang off this.

## B. Attributes, traits and the hidden role model

- **B1 [SPEC]** Attributes are rated **out of 100** (replacing the current 1..20 scale).
- **B2 [SPEC]** Starting spread at recruitment: **average 15-20**, a good attribute **~30**, a bad one
  **~10**.
  - **B2a [PROVISIONAL]** Concretely: roll each attribute around a mean of ~17, then apply the character's
...
- **B4 [SPEC]** Attributes are grouped into **Football-Manager-style categories**: Physical Attributes,
  ...
  - **B4a [PROVISIONAL]** Categories: **Physical**, **Mental**, **Social**, **Technical**, plus **Traits** (§B7,
    a different kind of thing: discrete, not 0..100).
- **B5 [SPEC]** The attribute set must cover every role's field, each in its respective area.
- **B6 [SPEC]** **Every human has every attribute**, including ones irrelevant to their role. There is no
  per-role attribute subset.
- **B7 [SPEC]** Some characters have a **unique trait granting a bonus**. Add a starter set now; the full
  trait system is **[TODO-LATER]**.
- **B8 [SPEC]** What each role actually needs is **internal to the game and never exposed to players**.
  **No star ratings**, no "suitability" score, no derived fit indicator. Players guess from the attributes
  and traits they can see.
  - **B8a [PROVISIONAL]** Architectural constraint: the role-requirement table lives **server-side only** and must
    never be shipped in the client bundle or returned by any API. A test asserts this.
- **B9 [SPEC]** **Research unlocks partial insight.** A librarian-like role/task that investigates hiring
...
- **D8 [SPEC]** **Reputation**: a _word_, not a number, applied to your group by its actions, and it
  ...
  **Treacherous**, **Collaborator**, **Reckless**, **Feared**, **Respected**.
  - **D8a [SPEC]** Add **all** of them now. Ones with no mechanic to drive them yet get an explicit
    **[TODO-LATER]** marker in code and are wired up when the mechanic lands.
- **D9 [PROVISIONAL]** The current MVP resources (`credits`, `power`, `data`, `alloy`) are **replaced**, not
...
- **E1 [SPEC]** Winning fights yields resources, **thematically matched to the mission** you ran.
- **E2 [SPEC]** Missions take real time. The troops / assigned people **are away** and **return** when
  it completes.
- **E3 [SPEC]** A **dedicated page** in the game shows every in-flight mission with its timer.
...
- **E8 [SPEC]** **Total elapsed = 2 × travel + mission time.**

## F. The player's own character (Overseer)

- **F1 [SPEC]** The Overseer has the **same attributes and traits** as everyone else.
- **F2 [SPEC]** You can **develop any of them**.
- **F3 [SPEC]** Board-named examples of what attributes do: **Charisma** → leading people, raising
...
- **F4 [SPEC]** Attributes are **bonuses**, and some **unlock new actions**. Worked example: researching
  something new, if the person on it has high enough **Imagination**, an option unlocks that would
  otherwise stay locked.
- **F5 [SPEC]** Second use: **modifiers on outcomes**. Worked example: a raid to steal resources:
  good **Speed** and **Stealth** on your character raise the team's success chance.
- **F6 [SPEC]** The initial Overseer **choice stays as it is today**, same four options, restated on
  the new attribute model (§B).

## G. Assignees

- **G1 [SPEC]** Under each officer you can place **Assignees**. They come from a **fungible pool**.
  They are interchangeable and have no individual identity.
- **G2 [SPEC]** The pool **grows with level**; on level-up you place the new ones you got.
- **G3 [SPEC]** Per-officer cap = **your level ÷ 2**: 1 at the start, **2 once you reach level 4**, and
  so on.
  - **G3a [PROVISIONAL]** Read as `max(1, floor(level / 2))`.
- **G4 [SPEC]** **"Reskilling"**: a process run by the **Professor** that lets you **reassign every
  assignee** at once.
- **G5 [SPEC]** Assignees add bonus to the **speed** and the **ability/power** of whatever the officer
  is doing.
- **G6 [SPEC]** **Hard** missions and internal processes **require an officer**. **Easy** ones can be run
  by a delegation of **assignees alone**, but slower and with a lower success chance.
- **G7 [SPEC]** Diminishing-returns bonus table (applies to both time reduction and power), by assignee
  count: **1→5% · 2→10% · 3→14.5% · 4→19% · 5→23.5% · 6→29% · 7→33% · 8→37% · 9→40% · 10→43% ·
  11→45% · 12→50%.** **12 assignees at 50% is the maximum.**
- **G8 [SPEC]** Grants: **start with 2**; **+1 per level**; **+1 extra at every 5th level** (5, 10,
  15, …).

## H. Recruitment: the Bar

- **H1 [SPEC]** Recruiting happens at a place called **the Bar**.
- **H2 [SPEC]** The Bar holds the currently available characters. The roster **refreshes every day** and
  is **the same for every player**.
  - **H2a [PROVISIONAL]** Therefore it is generated deterministically from the UTC date: one global roster, no
    per-player rolls.
- **H3 [SPEC]** Each character has **requirements to join you**, e.g. "at least this much infamy".
...
- **H4-H6 superseded [SPEC]** All three are removed. They were one idea between them, that a hire is a
  ...
  - **H4a [SPEC]** A character is now **their sheet and their perks**, both visible at the Bar before a
...
- **J1 [SPEC]** Players can come together into a **faction**: a team of **up to 5** members, with a
  name and a badge, on its own tab.
  - **J1a [PROVISIONAL]** The word previously meant two other things in the code (a district's own name, and
    a map holder kind) and both were renamed out of the way. A district's politics is now its
    **allegiance** (§A4).
- **J2 [SPEC]** The faction screen shows the members, and **the armies each of them currently
  holds**.
- **J3 [SPEC]** An ally's battles appear on the faction tab, and a member can **send units into
  ...
  - **J3a [PROVISIONAL]** Read as: a battle side is a list of contributors. Reinforcements go through the
...
- **J4 [SPEC]** Ranks: **leader**, **chief**, **member**.
  ...
  - **J4a [PROVISIONAL]** "Chief cannot touch chief" is the rule that needs stating: two chiefs able to
    remove each other turns a disagreement into a race. `canKick` takes both ranks for that reason.
- **J5 [SPEC]** A faction has a **name**, a **badge** and a **description**, and the name and badge
  ...
  - **J5a [PROVISIONAL]** 6 shapes x 12 colours x 6 patterns x 18 emblems x 12 colours, drawn from stored
    identifiers rather than stored as an image (`factions/badge.ts`, `FactionBadge.tsx`). No asset
    ships, and the maintainer's art policy is untouched: it is code-generated art.
- **J6 [SPEC]** **Invitation is the only way in, and it arrives as a message.** The invite lands in
  the recipient's inbox with a button on it; the button asks for confirmation before it joins.
  - **J6a [PROVISIONAL]** The message carries the invitation's id, so the button spends the same row the
...
- **J7 [SPEC]** **Leaving and disbanding.** Anybody can leave. A **leader** leaving disbands the
  ...
  - **J7a [PROVISIONAL]** Leaving used to be refused for a leader with people at the table, which left
...
- **J8 [SPEC]** **Team infamy** is **append-only**: the total infamy won in battle by people who
  ...
  - **J8a [PROVISIONAL]** An accumulator on the _faction_, credited from the two economies a fight produces
...
- **J9 [SPEC]** A **standings** screen, reached from the standing bar next to Actions. Two boards,
  **players** and **factions**, and a checkbox reading **My city only** that limits either one to
  ...
  - **J9a [PROVISIONAL]** There is one city (`CITIES`, Ashfall) so the two scopes list the same people
  ...
  - **J9b [PROVISIONAL]** Players rank on the infamy they hold, factions on §J8's append-only figure. Ties
    share a place and the next row skips the places used up.

- **J10 [SPEC]** Invitations are not bounded by geography: you can ask anybody, in any city.

## K. Messages and notifications [SPEC]

- **K1 [SPEC]** Two doors in the standing bar, **left of Battles**, each with an unread count.
- **K2 [SPEC]** **Messages**: write to another player or to your whole faction. Inbox, sent folder,
  reply, delete, unread until opened.
- **K3 [SPEC]** **Notifications**, in the manner of Grepolis: the game reporting what happened
  while you were away, every one linking to the thing it is about.
- **K4 [SPEC]** A filter in settings decides which kinds reach you.
  - **K4a [PROVISIONAL]** Applied at **write** time rather than read time, so switching a kind back on is a
...
- **K5 [SPEC]** The bell **is** the list. No tabs and no heading over it: an empty screen when
  nothing has happened, and the filters behind one drawn button in the corner marked
  **Preferences**.
- **K6 [SPEC]** Every notification **opens onto the thing it is about**, as a drawn report with
  labels and boxes rather than a line of prose.
  - **K6a [SPEC]** A mission's report says which job, which district, whether it worked, the
    battle result where there was one, and **what came back against what the job paid**.
  - **K6b [PROVISIONAL]** A notification carries a `subjectId` (`0053`) so it can name its mission rather
...
- **K7 [SPEC]** Sending units on a mission takes a typed number, steppers, and one-press **Half**
  and **Max**.

## I. Experience and levelling

- **I1 [SPEC]** XP comes from **missions**, **building things**, **quests**, and **fighting other
  players**.
- **I2 [SPEC]** Levelling up grants the bonuses above (recruit slots §H8) **and unlocks new things**.
  The assignee pool (§G8) and the per-officer cap (§G3) it also used to grant are both cut.
- **I3 [SPEC]** The set of unlocks beyond the above is **[TODO-LATER]**: the maintainer will file it
