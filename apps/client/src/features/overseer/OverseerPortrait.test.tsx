 ...
 *
 * So the box is the parent's, and the aimed crop is the part that keeps the reversal honest. The
 ...
 *
 * The anchor is `50% 0%`, and the **zero is the assertion**. For one day on 2026-09-22 it was
 * `50% -10%`, an attempt to answer "the top of the head is sometimes cropped" by sliding the
 * picture a tenth of the overflow lower. `0%` already puts the top edge of the delivery flush
 * with the top of the box, so a negative value pushes it below that line and opens a strip of
 * bare panel above the art, which is what the maintainer's screenshot of the file showed. There
 * is no picture above `0%` to slide in, so this pins the highest aim there is and no higher.
 */
describe('the fill box', () => {
  it('takes the whole frame and crops off the bottom rather than the head', () => {
    ...
    expect(container.querySelector('img')).toHaveStyle({ objectPosition: '50% 0%' });
  });
